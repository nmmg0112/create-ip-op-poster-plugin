...
For every `Pxx`, record the exact source filename, public name, unique-subject count, original group relationship, type, variant relationship, visible quality limitation, and intended use. Treat a public-safe example suffix such as `-版本A` or `-备选2` as file-management metadata unless the user says it is public copy. Keep the ledger hidden unless a conflict, missing source, identity ambiguity, or capability limitation changes the result.
